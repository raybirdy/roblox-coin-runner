...
**원인**: TRIGGER_INTERVAL=20s, VARIANCE=5s 설정으로 첫 이벤트 발동까지 최소 15초 소요.
테스트 세션이 15초 미만이면 이벤트가 한 번도 발동하지 않음. 코드 자체는 정상.

**수정 (commit e5c783d)**:
- `GameConstants.RANDOM_EVENTS.TRIGGER_INTERVAL`: 20 → 8 (±3s 범위, 첫 발동 5~11s)
- `SOLO_TRIGGER_INTERVAL`: 25 → 12
- `RandomEventController._onEventStart/_onEventEnd`에 print 로그 추가 (클라이언트 수신 확인)
