...
**구현 내용:**
- 각 플레이어마다 다른 Z 좌표에서 게임 진행
- 같은 서버에서 최대 5명 동시 플레이
- 다른 플레이어 캐릭터가 멀리 보임 (시각적 경쟁)
- 실시간 순위 표시

**장점:** 구현 간단, 현재 코드 활용

### Phase 2: 대결 모드 - 방 시스템 (v1.5)
...
| 모드 | 인원 | 규칙 | 우선순위 |
|------|------|------|----------|
| **스프린트** | 2~5인 | 먼저 목표 거리 도달 | Phase 1 |
| **서바이벌** | 2~5인 | 마지막까지 생존 | Phase 2 |
| **코인 배틀** | 2~5인 | 제한 시간 내 최다 코인 | Phase 2 |
| **릴레이** | 2~4인 팀 | 팀원 교대로 달리기 | Phase 3 |

### 우선순위 권장

```
v1.0 (현재) → v1.2 (Z축 레인) → v1.5 (방 시스템) → v2.0 (서버 분리)
     │              │                 │                  │
  싱글플레이    간단 멀티       대결 모드 완성      대규모 확장
  완성/배포      테스트          피드백 반영         필요시 진행
```

> **권장사항**: 먼저 싱글플레이어로 배포하여 사용자 반응을 확인한 후, 점진적으로 멀티플레이어 기능을 추가하는 것이 안전합니다.
...
- [x] 로비 시스템 (포탈 진입, 자유 이동)
- [x] 상점 시스템 (스킨, 시작 파워업, 부활권)
- [ ] 캐릭터 스킨 외형 적용
- [ ] 펫 시스템
- [ ] 추가 파워업 (2x 코인, 미니 사이즈)
- [ ] 움직이는 벽 장애물
